...
def test_onboard_tenant_end_to_end():
    ...
    # Import app after DATABASE_URL is set so engine is created pointing to the in-memory DB
    ...
    async def _check_subscription():
        async with engine.connect() as conn:
            res = await conn.execute(Base.metadata.tables['tenant_subscriptions'].select().where(Base.metadata.tables['tenant_subscriptions'].c.tenant_id == row['id']))
            return res.fetchone()
    ...
    async def _check_budget_pool():
        async with engine.connect() as conn:
            res = await conn.execute(Base.metadata.tables['budget_pools'].select().where(Base.metadata.tables['budget_pools'].c.tenant_id == row['id']))
            return res.fetchone()
