...
export default function PlatformAdminPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="px-4 py-2 rounded-xl bg-card border border-border-soft text-slate-600 text-xs font-bold hover:bg-slate-50 transition-all shadow-sm-sm"
          >
            Sytem Logs
          </button>
          <button 
            onClick={() => navigate('/platform-admin/subscriptions')}
            className="px-4 py-2 rounded-xl bg-indigo-600 border border-indigo-700 text-white text-xs font-bold hover:bg-indigo-700 transition-all shadow-sm-md"
          >
            Pricing Engine
          </button>
        </div>
          ...
          <ul className="mt-2 text-[11px] text-slate-600 space-y-1">
            {(stats.top_tenants || []).slice(0, 3).map(t => (
              <li key={t.id} className="flex justify-between border-b border-slate-50 pb-1">
                ...
                <span className="opacity-70 text-text-main">{t.recognitions} recs</span>
              </li>
            ))}
          </ul>
        </Card>
      ...
      {/* Tenant Management */}
      <div className="bg-card rounded-3xl border border-border-soft shadow-sm-sm overflow-hidden">
        <TenantManager 
          tenants={tenants} 
