...
function Avatar({ name }) {
  ...
  return (
    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center font-semibold text-sm">{initials}</div>
  )
}
    ...
    <div>
      {pages?.map((page, i) => (
        <div key={i} className="space-y-3">
          {page.map((r) => (
            <div key={r.id} className="p-4 bg-white rounded shadow flex items-start gap-3">
              <Avatar name={r.nominee_name || r.nominee_id} />
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium">{r.nominee_name || r.nominee_id}</div>
                    <div className="text-xs text-gray-500">Points: <span className="font-semibold">{r.points}</span></div>
                  </div>
                  <div>
